...
```
```{r, echo=FALSE}
prompri_1 <- mpg %>% select(manufacturer, cty) %>% group_by(manufacturer) %>% summarise_at(c("cty"), std.error) 
prompri_2 <- mpg %>% select(manufacturer, cty) %>% group_by(manufacturer)
```
```{r}
prom_2<- cbind(prom_1, prompri_1)
prom_2
```
